...
export default ({
    ...
    setup() {
        ...
        let franceCard = ref({ cardId: COUNTRY_CODES.FRANCE, cardType: CARD_TYPE.KINGDOM });
        let showBruges = ref(false);
        let brugesCard = ref({ cardId: CITY_NAMES.BRUGES, cardType: CARD_TYPE.PIECE, pieceId: "" });
        let showParis = ref(false);
        let parisCard = ref({ cardId: CITY_NAMES.PARIS, cardType: CARD_TYPE.PIECE, pieceId: "" });
        let showLyon = ref(false);
        let lyonCard = ref({ cardId: CITY_NAMES.LYON, cardType: CARD_TYPE.PIECE, pieceId: "" });

        const { showPiece } = useCard();
        const store = useStore();
        const france = computed(() => store.getters["kingdoms/getFrance"]);
        const refreshFrance = computed(() => store.getters["kingdoms/getRefreshFrance"]);
