...
export default ({
    ...
    setup() {
        ...
        let hungaryCard = ref({ cardId: COUNTRY_CODES.HUNGARY, cardType: CARD_TYPE.KINGDOM });
        let showBuda = ref(false);
        let budaCard = ref({ cardId: CITY_NAMES.BUDA, cardType: CARD_TYPE.PIECE, pieceId: '' });
        let showVarna = ref(false);
        let varnaCard = ref({ cardId: CITY_NAMES.VARNA, cardType: CARD_TYPE.PIECE, pieceId: '' });
        
        const { showPiece } = useCard();
        const store = useStore();
        const hungary = computed(() => store.getters["kingdoms/getHungary"]);
        const refreshHungary = computed(() => store.getters["kingdoms/getRefreshHungary"]);
