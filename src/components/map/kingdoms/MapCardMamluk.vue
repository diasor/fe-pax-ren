...
export default ({
    ...
    setup() {
        ...
        let mamlukCard = ref({ cardId: COUNTRY_CODES.MAMLUK, cardType: CARD_TYPE.KINGDOM });
        let showCyprus = ref(false);
        let cyprusCard = ref({ cardId: CITY_NAMES.CYPRUS, cardType: CARD_TYPE.PIECE, pieceId: '' });
        let showCairo = ref(false);
        let cairoCard = ref({ cardId: CITY_NAMES.CAIRO, cardType: CARD_TYPE.PIECE, pieceId: '' });
        let showRedSea = ref(false);
        let redSeaCard = ref({ cardId: CITY_NAMES.RED_SEA, cardType: CARD_TYPE.PIECE, pieceId: '' });

        const { showPiece } = useCard();
        const store = useStore();
        const mamluk = computed(() => store.getters["kingdoms/getMamluk"]);
        const refreshMamluk = computed(() => store.getters["kingdoms/getRefreshMamluk"]);
