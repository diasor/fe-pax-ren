...
export default ({
    ...
    setup() {
        ...
        let englandCard = ref({ cardId: COUNTRY_CODES.ENGLAND, cardType: CARD_TYPE.KINGDOM });
        let showLondon = ref(false);
        let londonCard = ref({ cardId: CITY_NAMES.LONDON, cardType: CARD_TYPE.PIECE, pieceId: "" });
        let showBordeaux = ref(false);
        let bordeauxCard = ref({ cardId: CITY_NAMES.BORDEAUX, cardType: CARD_TYPE.PIECE, pieceId: "" });
