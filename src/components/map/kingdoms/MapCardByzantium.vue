...
export default ({
    ...
    setup() {
        ...
        let byzantiumCard = ref({ cardId: COUNTRY_CODES.BYZANTIUM, cardType: CARD_TYPE.KINGDOM });
        let showTana = ref(false);
        let tanaCard = ref({ cardId: CITY_NAMES.TANA, cardType: CARD_TYPE.PIECE, pieceId: "" });
        let showCaffa = ref(false);
        let caffaCard = ref({ cardId: CITY_NAMES.CAFFA, cardType: CARD_TYPE.PIECE, pieceId: "" });
        let showTrebizond = ref(false);
        let trebizondCard = ref({ cardId: CITY_NAMES.TREBIZOND, cardType: CARD_TYPE.PIECE, pieceId: "" });

        const { showPiece } = useCard();
        const store = useStore();
        const byzantium = computed(() => store.getters["kingdoms/getByzantium"]);
        const refreshByzantium = computed(() => store.getters["kingdoms/getRefreshByzantium"]);
