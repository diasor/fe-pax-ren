...
export default {
    ...
    setup() {
        ...
        const storeWestMarket = computed(
            () => store.getters["markets/getWestMarket"]
        );
        const refreshWestMarket = computed(
            () => store.getters["markets/getRefreshWestMarket"]
        );
        const storeEastMarket = computed(
            () => store.getters["markets/getEastMarket"]
        );
        const refreshEastMarket = computed(
            () => store.getters["markets/getRefreshEastMarket"]
        );
        ...
        watch(refreshWestMarket, (refreshWestMarket) => {
            if (refreshWestMarket) {
                for (let i = 0; i < storeWestMarket.value.length; i++) {
                    const cardId =
                        i === 0 ? "PRW_BACK" : storeWestMarket.value[i].cardId;
                    const element = {
                        card: {
                            cardId,
        ...
        watch(refreshEastMarket, (refreshEastMarket) => {
            if (refreshEastMarket) {
                for (let i = 0; i < storeEastMarket.value.length; i++) {
                    const cardId =
                        i === 0 ? "PRE_BACK" : storeEastMarket.value[i].cardId;
                    const element = {
                        card: {
                            cardId,
