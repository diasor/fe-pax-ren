<template>
    <div>
        ...
        <div id="pr-map" class="map-container">
            <img
                src="/images/PR2_board_plain.jpg"
                usemap="#pr-map"
                @click="imageClick"
            />
            <map name="pr-map">
                <area
                    v-for="area in kingdomMap"
                    :key="area.title"
                    :id="area.id"
                    :alt="area.alt"
                    :title="area.title"
            ...
            <img
                v-if="showIslamico"
                :src="`/images/${KINGDOM_FILES[0].file}`"
                class="france"
                :style="franceDynamicClass"
            />
            <img
                v-if="showReformista"
                src="/images/Map Card Reformation01.png"
                class="england"
            />
            <img
                v-if="activateRenaissanceVictory"
                src="/images/PR2_Victorycards_active-01.png"
                class="renaissance-victory"
            />
        </div>
    </div>
</template>
...
import { KINGDOM_FILES } from "@/constants/cards";
